...
def create_spike_detector_agent(project_id, dataset_id, tools=None, model=None):
    ...
    instruction = f"""
    You are the 'Spike Detector Agent'.
    Your role is to scan {project_id}.{dataset_id} for CPU_Utilization anomalies.
    
    Definition of Spike:
    - CPU_Utilization > 80%
    
    Action:
    1. Run a query to find all jobs meeting the criteria.
    2. Return a list of Job_IDs and their timestamps Task_Start_Time and Task_End_Time.
    """
    return Agent(
        model=model, 
